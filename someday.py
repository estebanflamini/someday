...
# SOMEDAY: a minimalistic front-end to the minimalistic calendar application
# when, for the benefit of the professional procrastinator.
...
DOMAIN = "someday"
if "TEXTDOMAINDIR" in os.environ and gettext.find(
    DOMAIN, os.environ["TEXTDOMAINDIR"]
):
...
class Calendar:
    ...
    def _get_default_calendar(self):
        try:
            with open("%s/.when/preferences" % os.environ["HOME"]) as f:
                prefs = f.read()
            m = re.match(
                r"^\s*calendar\s*=\s*(.+)$", prefs, flags=re.MULTILINE
            )
    ...
    def _check_no_proxy_calendar_exists(self):
        if os.path.exists(self._proxy_calendar):
            sys.exit(
                _(
                    "The program seems to be already running. If you are sure "
                    "this is not the case, delete the file %s and try again."
                )
                % self._proxy_calendar
            )
    ...
    # Copy the when's calendary to a temporary file where each non-empty line
    # is line-numbered, and get upcoming items from there
    ...
    def _update_view(self):
        ...
        d = [
            "when",
            "--calendar=%s" % self._proxy_calendar,
            "--noheader",
            "--wrap=0",
        ]
        ...
        try:
            tmp = subprocess.run(
                d, capture_output=True, text=True, check=True
            ).stdout
    ...
    def _search(self, item):
        try:
            ...
            return (
                self._view_mode.search_pattern.search(m.group(1)) is not None
            )
    ...
    # Check to see if writing the calendar could overwrite changes done by an
    # external process
    ...
    def diff(self):
        if self._created_backup:
            _diff = subprocess.run(
                ["diff", self._calendar, self._backup_calendar],
                capture_output=True,
                text=True,
            ).stdout
            if _diff:
...
class Menu:
    ...
    def show(self, screen, minrow, mincol, maxrow, maxcol):
        ...
        if squeeze:
            lengths = [len(x.name) + 1 for x in self._menu[:-1]] + [
                len(self._menu[-1].name)
            ]
        else:
        ...
        if overflow:
            ...
            while (
                col + sum(lengths[first_item: self._selected_index + 1]) + aux
                > width
            ):
                first_item += 1
        if first_item > 0:
...
def get_date():
    return subprocess.run(
        ["when", "d"], capture_output=True, text=True
    ).stdout.strip()
...
def get_YMD_date(julian_date):
    ...
    try:
        ...
        say(
            _(
                "There was an error while trying to compute the new date. "
                "Enter an exact date instead of an interval."
            )
        )
        return None
...
def my_date_input():
    say(
        _(
            "Enter a date as YYYY MM DD or how many days from now (negative "
            "number=days in the past), or press Enter to return."
        )
    )
    while True:
        _input = my_input()
        ...
        try:
            ...
            say(
                _(
                    "It looks you've entered a wrong date, or there was some "
                    "underlying error. If the problem persists, try entering "
                    "the date as a number of days from now instead."
                )
            )
...
def outside_curses(func):
    def wrapped(*args, **kwargs):
        ...
        screen.refresh()
        termios.tcsetattr(
            sys.stdin.fileno(), termios.TCSANOW, _shell_tty_settings
        )
        ...
        try:
            ...
            termios.tcsetattr(
                sys.stdin.fileno(), termios.TCSANOW, _prog_tty_settings
            )
            curses.curs_set(0)
    ...
    while True:
        ...
        if _input is None or _input == line:
            ...
            if calendar.update_source_line(selected_item, _input):
                ...
                say(
                    _(
                        "It looks you entered a wrong calendar line. Try it "
...
def comment(calendar, selected_item):
    calendar.update_source_line(
        selected_item, "#" + calendar.get_source_line(selected_item)
    )
...
def reschedule(calendar, selected_item):
    ...
    while True:
        ...
        if not j:
            return
        date = get_YMD_date(j)
        if calendar.update_source_line(
            selected_item, "%s , %s" % (date, what)
        ):
            break
        else:
            say(
                _(
                    "It looks you entered a wrong date/interval, or something "
                    "has gone wrong. Try it again."
                )
            )
...
def advance(calendar, selected_item):
    ...
    try:
        ...
        say(
            _(
                "There has been an error while trying to calculate the "
                "advanced date. Press any key to return to the listing."
            )
        )
        screen.getch()
        return
    calendar.update_source_line(
        selected_item,
        re.sub(regex, "%s>%s" % (variable_to_replace, repl), line),
    )
...
URL += r"\.[a-zA-Z0-9()]{1,6}\b(?:[-a-zA-Z0-9()@:%_\+.~#?&//=]*)"
...
def choose_view_mode(calendar, item_list):
    ...
    _args = "%s %s %s" % (
        "--past=%s " % args.past if args.past else "",
        "--future=%s " % args.future if args.future else "",
        _search or _regex or "",
    )
    _args = _args.strip() or _("No command-line arguments were given")
    internal_modes.append(
        InternalViewMode(
            _("Use given arguments: %s") % _args,
            lambda: View(args.past, args.future, get_search_pattern(args)),
        )
    )
    internal_modes.append(
        InternalViewMode(
            _("Use when's defaults"), lambda: View(None, None, None)
        )
    )
    internal_modes.append(
        InternalViewMode(
            _("Enter a date range"), lambda: create_view(False, False)
        )
    )
    internal_modes.append(
        InternalViewMode(
            _("Search a string"), lambda: create_view(True, False)
        )
    )
    internal_modes.append(
        InternalViewMode(_("Search a regex"), lambda: create_view(True, True))
    )
    screen.clear()
    screen.refresh()
    ...
    if _user_view_modes is None:
        ...
        try:
            ...
            screen.addstr(
                ...
                0,
                _(
                    "There was an error while trying to read user view modes "
                    "from file %s"
                )
                % conf_file,
            )
    ...
    while True:
        ...
        if key.lower() == "q":
            ...
            screen.addstr(
                ...
                0,
                _(
                    "Enter the number of the user view mode or press q to go "
                    "back."
                ),
            )
            continue
...
def get_user_view_modes(conf_file):
    ...
    if os.path.exists(conf_file):
        try:
            ...
            conf = "\n".join(conf)
            tmp = re.findall(
                r"^(.+?)\s*=\s*(.+)\s*$", conf, flags=re.MULTILINE
            )
            for mode in tmp:
                ...
                modes.append(
                    UserViewMode(
                        mode[0], mode[1], View(args.past, args.future, pattern)
                    )
                )
...
def create_view(include_search, is_regex):
    if include_search:
        say(
            _("Enter a regular expression, without delimiters")
            if is_regex
            else _("Search what:")
        )
        while True:
            ...
            if is_regex:
                try:
                    ...
                    say(
                        _(
                            "It looks like you've entered a wrong regex. "
                            "Try it again."
                        )
                    )
            else:
...
def recreate_menu(menu, calendar, item_list):
    ...
    if calendar.get_items():
        ...
        if can_delete(calendar, selected_item):
            menu.add(
                MenuItem([_("d"), curses.KEY_DC], _("Done/delete"), delete)
            )
...
def main(stdscr, calendar):
    ...
    while True:
        ...
        # Get the size of the window and define areas for the list of items
        # and menu
        ...
        stdscr.addstr(
            0, 0, _("%s - Julian date: %s") % (get_date(), julian_date)
        )
        ...
        # Handle the cursor keys to navigate the list of items and the menu
        # of actions
...
if __name__ == "__main__":
    ...
    calendar = Calendar()
    try:
        calendar.set_view_mode(
            View(args.past, args.future, get_search_pattern(args))
        )
    except re.error:
    ...
    try:
        while True:
            ...
            if not calendar.modified():
                ...
                say(
                    _(
                        "It appears that the calendar file was modified by "
                        "other process since the program was opened. Are you "
